...
class FastaIndex:
    ...
    def load_seq(self, name: str) -> str:
        ...
        name_seq_tuple: List[Tuple[str, str]] = [
            next(read_fasta_stream(file, offset=offset))
            for (file, offset) in self.offsets.get(name, [])
        ]
        if len(name_seq_tuple) < 1:
            raise ValueError(
                f"Couldn't find seq {name} in FastaIndex {self.offsets.keys()}"
            )
...
def load_fasta_index(index_file: Path) -> FastaIndex:
    ...
    with index_file.open("rt") as inf:
        ...
        return FastaIndex(
            ...
            offsets={
                seq_name: [
                    (dir / filename, int(offset)) for (filename, _, offset) in triples
                ]
...
def read_fasta(p: Path, offset: int = None) -> Dict[str, str]:
    with open_with_gz(p) as inf:
        return {name: seq for (name, seq) in parse_fasta_stream(inf, offset=offset)}
