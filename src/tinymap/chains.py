...
class Anchor:
    ...
    def __lt__(self, other: "Anchor") -> bool:
        return self.reference_offset < other.reference_offset or (
            self.reference_offset == other.reference_offset
            and self.read_offset < other.read_offset
        )

    @property
    def read_end(self) -> int:
        return self.read_offset + self.k

    @property
    def reference_end(self) -> int:
        return self.reference_offset + self.k
    ...
    def gap_score(self, w: int, max_distance: int, other: "Anchor") -> float | None:
        ...
        if (
            other.read_offset <= self.read_offset
            or max(other.reference_diff(self), other.read_diff(self)) > max_distance
        ):
            return INF
        else:
...
class Region:
    ...
    def extract(self, v: str) -> str:
        return v[self.left : self.right]

    def contains(self, pt: int) -> bool:
        return self.left <= pt and pt < self.right

    def overlaps(self, other: "Region") -> bool:
        return self.contains(other.left) or other.contains(self.left)
...
class Chain:
    ...
    def chunks(self) -> List[List[int]]:
        ...
        return reduce(combiner, range(len(self.hits)), [])
    ...
    def overlaps(self, other: "Chain") -> bool:
        return self.contains_reference(
            other.reference_start
        ) or other.contains_reference(self.reference_start)
...
class Chainer:
    ...
    def compute_score(self, j: int, i: int) -> float:
        ...
        ), f"{j=} {self.hits[j]} and {i=} {self.hits[i]} were specified in the wrong order"
        msc = self.hits[j].match_score(self.params.w, self.hits[i])
        gsc = self.hits[j].gap_score(
            self.params.w, self.params.max_distance, self.hits[i]
        )
        sc = msc - gsc

        assert (
            self.scores[j] is not None
        ), f"Score for hit {j=} was None, {self.scores=}"
    ...
    def chain_align(self):
        ...
        for i in range(n):
            ...
            while (
                distant >= 0
                and self.hits[i].reference_diff(self.hits[distant])
            ...
            for j in range(i - 1, st - 1, -1):
                ...
                if score > NEG_INF and (max_j is None or score > max_score):
                    self.logger.log(
                        logging.DEBUG, f"new MAX {j=} {score=} -> {max_score=} {max_j=}"
                    )
                    max_score = score
                    max_j = j
                else:
                    self.logger.log(
                        logging.DEBUG, f"Iter {j=} {score=} vs. {max_j=} {max_score=}"
                    )
