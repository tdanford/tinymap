...
class AlignOp(Enum): 
    ...
    def advances_left(self) -> bool: return (self.value & 10) != 0
    def advances_right(self) -> bool: return (self.value & 6) != 0
    ...
    def arrow(self) -> str: 
        ...
        elif left > 0: return RIGHT_ARROW 
        elif right > 0: return LEFT_ARROW 
        else: return "\u21BB"
    
    def __lt__(self, other: 'AlignOp') -> bool: return self.value < other.value
    
    def prev_idx(self, idx: Tuple[int, int]) -> Tuple[int, int]: 
        (i, j) = idx 
        return (i - self.left_advance(), j - self.right_advance())
    
    def next_score(self, params: AlignmentParams, prev_score: float, new_gap_length: int) -> float: 
        if self == AlignOp.MATCH: 
            ...
            return prev_score + params.gap_open + (new_gap_length-1) * params.gap_extend
        else: 
            raise ValueError()
...
class Aligner: 
    ...
    @property
    def score(self) -> float: return self.scores[self.M-1][self.N-1]
    ...
    def score_matrix(self) -> str: 
        lines = list(reversed([' '.join(f"{s:0.1f}" for s in row) for row in self.scores])) 
        return '\n'.join(lines) 
    ...
    def align(self, left_only: bool = False) -> float: 
        ...
        if self.M > 1: 
            ...
            self.scores[1][0] = AlignOp.RIGHT_OPEN.next_score(self.params, self.scores[0][0], 1)
            gap_lengths[1][0] = 1 

        if self.N > 1: 
            self.ops[0][1] = AlignOp.LEFT_OPEN 
            self.scores[0][1] = AlignOp.LEFT_OPEN.next_score(self.params, self.scores[0][0], 1)
            gap_lengths[0][1] = 1 
                
        for i in range(2, self.M): 
            self.ops[i][0] = AlignOp.RIGHT_EXTEND 
            self.scores[i][0] = AlignOp.RIGHT_EXTEND.next_score(self.params, self.scores[0][0], i)
            gap_lengths[i][0] = i

        for i in range(2, self.N): 
            self.ops[0][i] = AlignOp.LEFT_EXTEND 
            self.scores[0][i] = AlignOp.LEFT_EXTEND.next_score(self.params, self.scores[0][0], i) 
            gap_lengths[0][i] = i
        
        for i in range(1, self.M): 
            for j in range(1, self.N): 
                left_gap_op = AlignOp.RIGHT_EXTEND if self.ops[i-1][j].is_right_gap() else AlignOp.RIGHT_OPEN
                left_gap_length = gap_lengths[i-1][j] + 1 if left_gap_op == AlignOp.RIGHT_EXTEND else 1 
                prev_left_score = self.scores[i-left_gap_length][j]
                left_gap_score = left_gap_op.next_score(self.params, prev_left_score, left_gap_length)

                right_gap_op = AlignOp.LEFT_EXTEND if self.ops[i][j-1].is_left_gap() else AlignOp.LEFT_OPEN
                right_gap_length = gap_lengths[i][j-1] + 1 if right_gap_op == AlignOp.LEFT_EXTEND else 1
                prev_right_score = self.scores[i][j-right_gap_length]
                right_gap_score = right_gap_op.next_score(self.params, prev_right_score, right_gap_length)

                mm_op = AlignOp.MATCH if self.left_seq[i-1] == self.right_seq[j-1] else AlignOp.MISMATCH 
