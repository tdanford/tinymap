...
class Alignment:
    ...
    def alignable_chunks(self, buffer: int = 20) -> List[Tuple[Region, Region]]:
        ...
        for i in range(len(chunks)):
            ...
            ref_regions.append(
                self.reference_region(left_anchor, right_anchor, buffer=buffer)
            )
            ...
            ref_regions.append(
                Region(left_anchor.reference_offset, right_anchor.reference_end)
            )
        ...
        return [
            (rer, rfr)
            for (rer, rfr) in zip(read_regions, ref_regions)
    ...
    def alignable_regions(self, buffer: int = 20) -> List[Tuple[Region, Region]]:
        ...
        ref_regions = [
            self.reference_region(r[0], r[1], buffer=buffer) for r in anchor_pairs
        ]
        return [
            (rer, rfr)
            for (rer, rfr) in zip(read_regions, ref_regions)
    ...
    def read_region(self, left_anchor: Anchor, right_anchor: Anchor) -> Region:
        if left_anchor is None and right_anchor is None:
            raise ValueError(
                f"Both left and right anchors can't be None, when defining a region"
            )
    ...
    def reference_region(
        self, left_anchor: Anchor, right_anchor: Anchor, buffer: int = 20
    ) -> Region:
        if left_anchor is None and right_anchor is None:
            raise ValueError(
                f"Both left and right anchors can't be None, when defining a region"
        ...
        if left_anchor is None:
            ...
            return Region(
                max(0, right_anchor.reference_offset - left_edge),
                right_anchor.reference_offset,
            )
        elif right_anchor is None:
            right_edge = (len(self.read_seq) - left_anchor.read_end) + buffer
            return Region(
                left_anchor.reference_end, left_anchor.reference_end + right_edge
            )
        else:
...
class TinymapIndex:
    ...
    def index_seq(self, seq_name: str, seq: str):
        ...
        self.seq_indices[seq_name] = TinymapSeqIndex(
            w=self.w,
            k=self.k,
            seq_name=seq_name,
            seq=seq,
    ...
    def base_align(self, align: Alignment) -> Tuple[str, str]:
        ...
        return ("".join(x[0] for x in aligns), "".join(x[1] for x in aligns))

    def align(self, read: Read) -> List[Alignment]:
        self.logger.debug(f"Aligning {read}")
        hits = [
            align for (_, idx) in self.seq_indices.items() for align in idx.align(read)
        ]
        hits.sort()
...
class TinymapSeqIndex:
    ...
    def load(p: Path) -> "TinymapSeqIndex":
        ...
        return TinymapSeqIndex(w, k, seq_name, seq, seed_df)
...
def make_make_anchor(k: int) -> Callable[[Dict], Anchor]:
    def maker(row) -> Anchor:
        return Anchor(
            read_offset=row["offset_read"], reference_offset=row["offset_ref"], k=k
        )
